(function initWorkflowUI(globalScope) {
  ...
  const _paineis = [
    'tarefas','instancias','solicitacoes','iniciar','modelagem',
  ...
  function wfNavWorkflow(painel) {
    ...
    // Destaca tab ativa
    ['tarefas','instancias','solicitacoes','modelagem','formularios'].forEach(t => {
  ...
  // Item 2 — SLA badge helper
  function _wfSlaHtml(prazo) {
    if (!prazo) return '';
    const prazoDate = prazo.toDate ? prazo.toDate() : (prazo instanceof Date ? prazo : new Date(prazo.seconds * 1000));
    const agora = Date.now();
    const diff = prazoDate.getTime() - agora;
    const horas = Math.round(diff / 3600000);
    if (diff < 0) return `<span class="wf-sla-badge wf-sla-vencido">&#9888; Vencido há ${Math.abs(horas)}h</span>`;
    if (horas < 4) return `<span class="wf-sla-badge wf-sla-vencendo">&#8987; ${horas}h restantes</span>`;
    const dateStr = prazoDate.toLocaleDateString('pt-BR');
    return `<span class="wf-sla-badge wf-sla-ok">&#128197; ${_esc(dateStr)}</span>`;
  }
  ...
  async function wfAbrirTarefa(tarefaId) {
    ...
    // Cabeçalho redesenhado (Item 2)
    const tituloEl = document.getElementById('wf-exec-titulo');
    if (tituloEl) tituloEl.textContent = tarefa.etapa_nome || tarefa.etapa_modelo_id;
    const procNomeEl = document.getElementById('wf-exec-processo-nome');
    if (procNomeEl) procNomeEl.textContent = tarefa.processo_nome || tarefa.instancia_id || '';
    const slaEl = document.getElementById('wf-exec-sla-badge');
    if (slaEl) slaEl.innerHTML = tarefa.prazo ? _wfSlaHtml(tarefa.prazo) : '';
    ...
    if (papelEl) papelEl.innerHTML = tarefa.papel_responsavel ? `Seu papel: ${_badge(tarefa.papel_responsavel, '#6366f1')}` : '';

    // Responsável
    const respEl = document.getElementById('wf-exec-responsavel');
    if (respEl) {
      const nomeResp = tarefa.responsavel_uid
        ? ((globalScope.USUARIOS || []).find(u => u.uid === tarefa.responsavel_uid || u.id === tarefa.responsavel_uid)?.nome || tarefa.responsavel_uid)
        : '';
      respEl.textContent = nomeResp ? `Responsável: ${nomeResp}` : '';
    }
    ...
    if (etapa?.instrucoes && instrDiv) {
      instrDiv.textContent = etapa.instrucoes;
    ...
    // Dados consolidados — grid de key-value (Item 2)
    ...
    if (dadosEntradas.length && dadosAntEl && dadosAntConteudo) {
      dadosAntConteudo.innerHTML = dadosEntradas.map(([k, v]) =>
        `<div style="background:var(--surf2);border:1px solid var(--bdr);border-radius:8px;padding:8px 12px">
          <div style="font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.04em;color:var(--ink3);margin-bottom:2px">${_esc(k)}</div>
          <div style="font-size:13px;color:var(--ink)">${_esc(String(v))}</div>
        </div>`
      ).join('');
    ...
    // Item 6 — Histórico inline
    const histEl = document.getElementById('wf-exec-historico-lista');
    if (histEl && tarefa.instancia_id) {
      histEl.innerHTML = '<div style="color:var(--ink3);font-size:12px">Carregando…</div>';
      try {
        const { where: w2, orderBy } = globalScope.fb();
        const hist = await _getAll('wf_historico_workflows',
          w2('instancia_id', '==', tarefa.instancia_id),
          orderBy('_criado_em', 'desc'),
        );
        const recentes = hist.slice(0, 8);
        histEl.innerHTML = recentes.map(h => {
          ...
          const autorNome = h.usuario_uid && h.usuario_uid !== 'sistema'
            ? ((globalScope.USUARIOS || []).find(u => u.uid === h.usuario_uid || u.id === h.usuario_uid)?.nome || h.usuario_uid)
            : 'Sistema';
          return `<div style="padding:8px 0 8px 12px;border-bottom:1px solid var(--bdr);font-size:12px">
            <div style="font-weight:500;color:var(--ink)">${_esc(h.descricao || h.tipo_evento || '')}</div>
            <div style="color:var(--ink4);margin-top:2px;font-size:11px">${_esc(autorNome)} · ${_esc(ts)}</div>
          </div>`;
        }).join('') || '<div style="color:var(--ink3);font-size:12px">Sem histórico.</div>';
      } catch (_e) {
        histEl.innerHTML = '';
      }
    }

    // Comentários placeholder (Item 2)
    const comentariosEl = document.getElementById('wf-exec-comentarios');
    if (comentariosEl) comentariosEl.innerHTML = '';
  ...
  async function _avancarFluxo(instancia, etapaOrigemId, acao) {
    ...
    if (proxEtapa.tipo === 'fim') {
      await _updateDoc('wf_instancia_processos', instancia.id, {
        ...
        etapa_atual_nome: null, responsavel_atual_nome: null, prazo_atual: null,
      });
  ...
  async function _criarTarefa(instancia, etapa) {
    ...
    const tarefaId = await _addDoc('wf_tarefa_workflows', {
      ...
      etapa_nome: etapa.nome || etapa.id,
      responsavel_uid: responsavel,
      papel_responsavel: etapa.responsavel_tipo || null,
      exige_parecer: !!etapa.exige_parecer,
      status: 'pendente',
      prazo,
      dados_formulario: {},
      acao_tomada: null,
    ...
    // Item 3 — Denormaliza campos na instância para exibição rápida nas listas
    const nomeResp = responsavel
      ? ((globalScope.USUARIOS || []).find(u => u.uid === responsavel || u.id === responsavel)?.nome || responsavel)
      : null;
    await _updateDoc('wf_instancia_processos', instancia.id, {
      etapa_atual_nome: etapa.nome || etapa.id,
      responsavel_atual_nome: nomeResp || null,
      prazo_atual: prazo || null,
    });
  ...
  async function wfCarregarInstancias() {
    ...
    try {
      ...
      el.innerHTML = instancias.map(i => {
        const slaHtml = i.prazo_atual ? _wfSlaHtml(i.prazo_atual) : '';
        const etapaHtml = i.etapa_atual_nome && i.status === 'em_andamento'
          ? `<div style="font-size:12px;color:var(--ink3);margin-top:2px">Etapa: <strong>${_esc(i.etapa_atual_nome)}</strong></div>` : '';
        const respHtml = i.responsavel_atual_nome
          ? `<div style="font-size:12px;color:var(--ink3);margin-top:2px">Responsável: <strong style="color:var(--ink2)">${_esc(i.responsavel_atual_nome)}</strong></div>` : '';
        return _card(`
  ...
  // Item 1 — Modal de formulário de abertura
  async function _wfAbrirModalInicioForm(formularioId, nomeModelo, callback) {
    const el = document.getElementById('wf-modal-inicio-form');
    if (!el) { callback({}); return; }

    const tituloEl = document.getElementById('wf-inicio-form-titulo');
    const subEl = document.getElementById('wf-inicio-form-subtitulo');
    const camposEl = document.getElementById('wf-inicio-form-campos');

    if (tituloEl) tituloEl.textContent = 'Formulário de abertura';
    if (subEl) subEl.textContent = nomeModelo;
    if (camposEl) camposEl.innerHTML = '<div style="color:var(--ink3);font-size:13px">Carregando formulário…</div>';

    el.style.display = 'flex';
    el._inicioFormCallback = callback;
    el._inicioFormId = formularioId;

    try {
      const schema = await _getDoc('wf_formulario_modelos', formularioId);
      if (!schema || !camposEl) { callback({}); el.style.display = 'none'; return; }
      el._inicioFormSchema = schema;
      if (typeof globalScope.wfRenderizarFormulario === 'function') {
        camposEl.innerHTML = '';
        const formEl = globalScope.wfRenderizarFormulario(schema, {});
        camposEl.appendChild(formEl);
      } else {
        camposEl.innerHTML = '<div style="color:var(--red);font-size:13px">Módulo de formulários não disponível.</div>';
      }
    } catch (_e) {
      if (camposEl) camposEl.innerHTML = '<div style="color:var(--red);font-size:13px">Erro ao carregar formulário.</div>';
    }
  }

  function wfConfirmarInicioForm() {
    const el = document.getElementById('wf-modal-inicio-form');
    if (!el) return;
    const schema = el._inicioFormSchema;
    const camposEl = document.getElementById('wf-inicio-form-campos');
    let dados = {};
    if (schema && camposEl) {
      const formEl = camposEl.querySelector('.wf-form');
      if (formEl && typeof globalScope.wfColetarDadosFormulario === 'function') {
        const resultado = globalScope.wfColetarDadosFormulario(formEl, schema.campos || []);
        if (!resultado.valido) return;
        dados = resultado.dados;
      }
    }
    el.style.display = 'none';
    if (typeof el._inicioFormCallback === 'function') el._inicioFormCallback(dados);
  }
  ...
  function wfCancelarInicioForm() {
    ...
    if (!el) return;
    el.style.display = 'none';
    if (typeof el._inicioFormCallback === 'function') el._inicioFormCallback(null);
  }
  ...
  async function wfIniciarProcesso() {
    ...
    try {
      ...
      // Item 1 — Se o modelo tem formulario_inicio_id, mostrar modal antes de criar
      const formularioInicioId = modelo.formulario_inicio_id || null;

      const _prosseguirComDados = async (dadosInicio) => {
        if (dadosInicio === null) return; // cancelado
        ...
        const tituloFinal = titulo || `${modelo.nome} — ${new Date().toLocaleDateString('pt-BR')}`;
        const instanciaId = await _addDoc('wf_instancia_processos', {
          ...
          solicitante_uid: uid,
          dados_consolidados: dadosInicio || {},
          concluido_em: null,
        ...
        if (etapaInicial) {
          await _criarTarefa({ id: instanciaId, titulo: tituloFinal, processo_modelo_id: modeloId, solicitante_uid: uid }, etapaInicial);
        }
      ...
      if (formularioInicioId) {
        await _wfAbrirModalInicioForm(formularioInicioId, modelo.nome, _prosseguirComDados);
      } else {
        await _prosseguirComDados({});
      }
    } catch (e) {
  ...
  async function wfCarregarSolicitacoes() {
    ...
    el.innerHTML = '<div style="color:var(--ink3);font-size:14px">Carregando…</div>';
    try {
      const { where, orderBy, limit, query, collection, getDocs } = globalScope.fb();
      const snap = await getDocs(query(
        collection(_db(), 'wf_instancia_processos'),
        where('solicitante_uid', '==', uid),
        orderBy('_criado_em', 'desc'),
        limit(50),
      ));
      const instancias = snap.docs.map(d => ({ id: d.id, ...d.data() }));
      if (!instancias.length) {
        el.innerHTML = '<div style="color:var(--ink3);font-size:14px;padding:16px 0">Você ainda não iniciou nenhum processo.</div>';
        return;
      }
      ...
      el.innerHTML = instancias.map(i => {
        const slaHtml = i.prazo_atual ? _wfSlaHtml(i.prazo_atual) : '';
        const resp = i.responsavel_atual_nome
          ? `<div style="font-size:12px;color:var(--ink3);margin-top:4px">Responsável: <strong style="color:var(--ink2)">${_esc(i.responsavel_atual_nome)}</strong></div>`
          : '';
        const etapa = i.etapa_atual_nome && i.status === 'em_andamento'
          ? `<div style="font-size:12px;color:var(--ink3);margin-top:2px">Etapa: <strong>${_esc(i.etapa_atual_nome)}</strong></div>`
          : '';
        const criado = i._criado_em?.seconds ? new Date(i._criado_em.seconds * 1000).toLocaleDateString('pt-BR') : '';
        return _card(`
          <div style="font-weight:600;font-size:14px;margin-bottom:6px">${_esc(i.titulo || i.id)}</div>
          <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:6px">
            ${_badge(statusLabels[i.status] || i.status, statusCores[i.status] || '#6b7280')}
            ${slaHtml}
          </div>
          ${etapa}${resp}
          ${criado ? `<div style="font-size:11px;color:var(--ink4);margin-top:8px">Iniciado em ${_esc(criado)}</div>` : ''}
          <div style="margin-top:10px">
            <button type="button" class="btn btn-sm" onclick="wfAbrirHistorico('${_esc(i.id)}','${_esc(i.titulo || i.id)}','${_esc(i.status)}')">Ver acompanhamento</button>
          </div>
        `);
      }).join('');
    } catch (e) {
  ...
  // ── Item 4: Labels customizados de ações (helper para o config-modelo) ────
  // Os labels são configurados por etapa em modelo.etapas_config[etapaId].acoes_labels
  // e aplicados em _criarTarefa via tarefa.acoes_labels.

  // ── Item 7: Simulação de fluxo ────────────────────────────────────────────
  function wfDesignerSimular() {
    const el = document.getElementById('wf-modal-simulacao');
    if (!el) return;
    const camposEl = document.getElementById('wf-sim-campos');
    const resEl = document.getElementById('wf-sim-resultado');
    if (resEl) resEl.innerHTML = '';

    // Para este módulo simplificado, usa etapas e transições do modelo atual
    const modelo = _st.modeloAtual;
    if (!modelo) {
      if (camposEl) camposEl.innerHTML = '<div style="color:var(--amber);font-size:13px">Abra um modelo para simular.</div>';
      el.style.display = 'flex';
      return;
    }

    // Coleta campos das transições (condições como "aprovado"/"rejeitado")
    const transicoes = _st._transicoesCache || [];
    const camposUsados = new Set();
    transicoes.forEach(t => { if (t.condicao && t.condicao !== 'sempre') camposUsados.add('acao'); });

    if (camposEl) {
      if (!camposUsados.size) {
        camposEl.innerHTML = '<div style="color:var(--ink3);font-size:13px">Este modelo usa transições automáticas — o caminho é linear.</div>';
      } else {
        camposEl.innerHTML = [...camposUsados].map(c => `
          <div style="margin-bottom:10px">
            <label class="lbl">${_esc(c === 'acao' ? 'Ação tomada em cada etapa' : c)}</label>
            <input type="text" class="fi" id="wf-sim-campo-${_esc(c)}" placeholder="${c === 'acao' ? 'Ex: aprovado, rejeitado, Avançar' : ''}">
          </div>
        `).join('');
      }
    }

    el._simModelo = modelo;
    el._simTransicoes = transicoes;
    el._simCampos = [...camposUsados];
    el.style.display = 'flex';
  }

  async function wfExecutarSimulacao() {
    const el = document.getElementById('wf-modal-simulacao');
    const resEl = document.getElementById('wf-sim-resultado');
    if (!el || !resEl) return;

    const modelo = el._simModelo;
    if (!modelo) { resEl.innerHTML = '<div style="color:var(--red)">Modelo não disponível.</div>'; return; }

    const camposUsados = el._simCampos || [];
    const dados = {};
    camposUsados.forEach(c => {
      const inp = document.getElementById(`wf-sim-campo-${c}`);
      if (inp) dados[c] = inp.value;
    });

    // Simula caminho pelas transições
    const { where } = globalScope.fb();
    let transicoes = el._simTransicoes;
    if (!transicoes || !transicoes.length) {
      try {
        transicoes = await _getAll('wf_transicao_fluxos', where('processo_modelo_id', '==', modelo.id));
      } catch (_e) { transicoes = []; }
    }

    const etapas = _st.etapasModelo || [];
    const etapaInicial = etapas.find(e => e.id === modelo.etapa_inicial) || etapas[0];
    if (!etapaInicial) { resEl.innerHTML = '<div style="color:var(--red)">Modelo sem etapa inicial.</div>'; return; }

    const acaoSim = dados.acao || 'Avançar';
    const caminho = [etapaInicial];
    let atual = etapaInicial;
    let passos = 0;
    while (atual && atual.tipo !== 'fim' && passos < 20) {
      const trans = transicoes.find(t =>
        t.etapa_origem_id === atual.id &&
        (t.condicao === 'sempre' || t.label === acaoSim || t.condicao === acaoSim)
      ) || transicoes.find(t => t.etapa_origem_id === atual.id);
      if (!trans) break;
      const prox = etapas.find(e => e.id === trans.etapa_destino_id);
      if (!prox || caminho.find(c => c.id === prox.id)) break;
      caminho.push(prox);
      atual = prox;
      passos++;
    }

    const icones = { inicio:'▶', tarefa:'📋', aprovacao:'✅', fim:'⏹', gateway:'◆' };
    resEl.innerHTML = `
      <div style="font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:var(--ink3);margin-bottom:10px">Caminho simulado (${caminho.length} etapa${caminho.length !== 1 ? 's' : ''})</div>
      <div style="display:flex;flex-direction:column;gap:6px">
        ${caminho.map((n, i) => {
          const icone = icones[n.tipo] || '•';
          const cor = n.tipo === 'fim' ? '#10b981' : n.tipo === 'inicio' ? '#6366f1' : 'var(--blue)';
          return `<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;border-radius:8px;background:var(--surf2);border:1px solid var(--bdr)">
            <span style="font-size:16px;flex-shrink:0">${icone}</span>
            <div style="flex:1">
              <div style="font-size:13px;font-weight:600;color:${cor}">${_esc(n.nome || n.id)}</div>
              <div style="font-size:11px;color:var(--ink3)">Etapa ${i + 1} · ${_esc(n.tipo)}</div>
            </div>
          </div>`;
        }).join('<div style="text-align:center;color:var(--ink4);font-size:16px;margin:-2px 0">&#8595;</div>')}
      </div>`;
  }

  // Funções auxiliares de comentários/anexos — stubs que evitam erro se o módulo completo não estiver carregado
  function wfAnexarArquivos() {}
  function wfEnviarComentario() {}
  ...
  Object.assign(globalScope, {
    ...
    // Item 2
    _wfSlaHtml,
    // Item 5 (already wfCarregarSolicitacoes above)
    // Item 7
    wfDesignerSimular,
    wfExecutarSimulacao,
    // Stubs de comentários/anexos
    wfAnexarArquivos,
    wfEnviarComentario,
    wfCancelarResposta,
    wfCarregarModelos,
