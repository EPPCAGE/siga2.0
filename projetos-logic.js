  ...
  tarefas.forEach((t, i) => {
    ...
    html += `<tr style="${rowBg}">
      ...
      <td style="padding:5px 8px;border-bottom:1px solid #eaecf3;padding-left:${8+indent}px;${bold};${strike}">
        ${wrapNames ? `<textarea class="proj-task-name-input proj-task-name-textarea" rows="1" placeholder="Nova tarefa" aria-label="Nome da tarefa" oninput="projAutosizeTaskNameTextarea(this)" onchange="projUpdateTarefa('${path}','nome',this.value.trim()||'Nova tarefa')">${projEsc(t.nome||'Nova tarefa')}</textarea>` : `<input type="text" class="proj-task-name-input" value="${projEsc(t.nome||'Nova tarefa')}" placeholder="Nova tarefa" aria-label="Nome da tarefa" onchange="projUpdateTarefa('${path}','nome',this.value.trim()||'Nova tarefa')" onkeydown="if(event.key==='Enter'){event.preventDefault();this.blur();}">`}
      </td>
...
function projAutosizeTaskNameTextarea(el) {
  if(!el) return;
  el.style.height = 'auto';
  el.style.height = Math.max(30, el.scrollHeight) + 'px';
}

function projAutosizeTaskNameTextareas() {
  document.querySelectorAll('.proj-task-name-textarea').forEach(projAutosizeTaskNameTextarea);
}
